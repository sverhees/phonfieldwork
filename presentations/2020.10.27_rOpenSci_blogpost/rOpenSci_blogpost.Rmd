...
3. Elicit the list of stimuli from speakers who signed an Informed Consent statement, agreeing to participate in the experiment and to be recorded on audio and/or video. Keep an eye on recording settings: sampling rate, resolution (bit), and number of channels should be the same across all recordings.
...
The phonfieldwork package was created to help with items 3, 5 and 8, and partially with 4. If you are interested in the whole pipeline, please read the whole [Get started section](https://ropensci.github.io/phonfieldwork/articles/phonfieldwork.html) in the package documentation. Below I will show some specific features.
...
There is also an option to work with `.flextext` files from FLEx, but since this is not actually connected with phonetics, I will skip this part.

### Sound visualization

Sound visualization is a common task that could be solved with different programs and R packages, but it is also possible to do with `phonfieldwork`:
...
A specific feature of `phonfieldwork` is that it is possible to zoom in to some part of the sound with the spectrogram (time in the `zoom` argument is present in seconds):

```{r}
draw_sound(file, zoom = c(0.2, 0.4))
```

It is also possible to visualize any sound annotation format that was converted to a dataframe in the previous section:
...
If you have folders with small sound chunks and thir visualizations it is possible to create a sound viewer like [this](https://ropensci.github.io/phonfieldwork/additional/stimuli_viewer.html). This is done using the `create_viewer()` function:
...
If you are familiar with my package `lingtypology` Moroz (2017) for interactive linguistic map generation and API for typological databases, there is a good news for you: it is possible to connect the two pacakages and create an interactive map that shares the same hear and view buttons. Here is [an example](https://ropensci.github.io/phonfieldwork/additional/stimuli_viewer2.html).
...
* the participants of seminars at the School of linguistics and Linguistic Convergence Laboratory at HSE, Moscow, where I first presented this package;
* my friends Neige Rochant and Samira Verhees for sharing their data and problems with me, which has improved `phonfieldwork`.

## Announcement

Dear all,

It’s my pleasure to announce a new stable release of phonfieldwork v.0.0.10 package for R that just passed the rOpenSci review. The main goal of this package is to provide a huge variety of tools that can be used by phonetic researchers in the field and in laboratories. The package helps to collect, annotate and convert different sound annotation formats to tables. It is also possible to create sound viewers (like [this](https://ropensci.github.io/phonfieldwork/additional/stimuli_viewer.html)), which can be a handy tool for data analysis and data sharing processes in phonetics. The process of work with this package is documented [here](https://ropensci.github.io/phonfieldwork/), but I would be happy to answer any of your questions via github issues or mail.
